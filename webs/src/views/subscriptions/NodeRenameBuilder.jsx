...
export default function NodeRenameBuilder({ value, onChange }) {
    ...
    // 同步规则到父组件
    const syncRule = useCallback((items) => {
    ...
    // 生成预览
    const preview = ruleItems.map(item => {
                                            ...
                                            {(provided, snapshot) => (
                                                <Fade in>
                                                    <Chip
                                                        ...
                                                        label={
                                                            item.type === 'variable'
                                                                ? getVariableLabel(item.value)
                                                                : `"${item.value}"`
                                                        }
                                                        onDelete={() => handleRemoveItem(item.id)}
                                                        deleteIcon={<DeleteOutlineIcon sx={{ fontSize: 16 }} />}
                                                        sx={{
                                                            bgcolor: item.type === 'variable'
                                                                ? `${getVariableColor(item.value)}20`
                                                                : theme.palette.mode === 'dark' ? '#333' : '#e0e0e0',
                                                            color: item.type === 'variable'
                                                                ? getVariableColor(item.value)
                                                                : 'text.primary',
                                                            fontWeight: 600,
                                                            border: '1px solid',
                                                            borderColor: item.type === 'variable'
                                                                ? `${getVariableColor(item.value)}40`
                                                                : 'divider',
