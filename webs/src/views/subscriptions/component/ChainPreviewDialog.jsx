                ...
                <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1.5}>
                    <Stack direction="row" alignItems="center" spacing={1} flexWrap="wrap">
                        <Typography
                            variant="subtitle1"
                            fontWeight={700}
                            sx={{ textDecoration: isFullyCovered ? 'line-through' : 'none' }}
                        >
                            {rule.ruleName || '未命名规则'}
                        </Typography>
                        {!rule.enabled && <Chip label="已禁用" size="small" color="default" />}
                        {rule.enabled && isFullyCovered && (
                            <Chip label="已被覆盖" size="small" color="warning" variant="outlined" />
                        )}
                        {rule.enabled && !isFullyCovered && rule.effectiveNodes > 0 && (
                            <Chip
                                label={`生效 ${rule.effectiveNodes} 节点`}
                                size="small"
                                color="success"
                                variant="outlined"
                            />
                            ...
                            <TableRow key={node.nodeId} sx={{ opacity: node.unmatched ? 0.5 : 1 }}>
                                <TableCell>
                                    <Stack direction="row" alignItems="center" spacing={0.5}>
                                        ...
                                        <Typography variant="body2" sx={{ maxWidth: isMobile ? 100 : 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                            {node.nodeName}
                                        </Typography>
                                    </Stack>
                                </TableCell>
                        ...
                        {tab === 0 && (
                            <Box>
                                <Typography variant="caption" color="text.secondary" display="block" mb={2}>
                                    💡 规则按顺序匹配，每个节点只会应用第一个匹配的规则
                                </Typography>
                                {rules.map((rule, index) => (
                                    <RuleChainFlow key={rule.ruleId || index} rule={rule} isMobile={isMobile} theme={theme} />
                                ))}
                            </Box>
                        )}
