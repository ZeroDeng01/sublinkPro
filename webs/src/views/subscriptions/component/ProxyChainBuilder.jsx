 ...
 * 用于配置入口代理的类型和参数
 */
export default function ProxyChainBuilder({
    value = [],
    onChange,
    nodes = [],
    fields = [],
                            ...
                            {item.type === 'specified_node' && (
                                <Autocomplete
                                    ...
                                    getOptionLabel={(option) =>
                                        `${option.name || option.linkName} (${option.linkCountry || '未知'})`
                                    }
                                    value={nodes.find((n) => n.id === item.nodeId) || null}
                                    onChange={(e, newValue) =>
                                        handleItemChange(index, { nodeId: newValue?.id })
                                    }
                                    renderInput={(params) => (
                                        <TextField {...params} label="选择节点" />
                                    )}
                                    renderOption={(props, option) => (
                                        <li {...props} key={option.id}>
                                            <Stack direction="row" spacing={1} alignItems="center">
                                                ...
                                                <Chip
                                                    label={option.linkCountry || '未知'}
                                                    size="small"
                                                    variant="outlined"
                                                />
                                                <Chip
                                                    label={option.protocol || 'unknown'}
                                                    size="small"
                                                    color="info"
                                                    variant="outlined"
                                                />
